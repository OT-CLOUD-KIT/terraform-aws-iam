...
#     trust_policy = object({   # required trust policy
#       policy_template_file = string # Path to json or json.tmpl file of policy
#       policy_template_vars = map(string)    # Policy template variables {key = val, ...}
#     })
...
variable "create_rds_role" {
  description = "Provide permission to BuildPiper to create RDS Resource"
  type        = bool
  default     = false
}
